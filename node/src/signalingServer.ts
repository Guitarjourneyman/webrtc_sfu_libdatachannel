...
type SignalRequest =
  | { id?: string; action: 'createViewer'; peerId: string; sdp?: string }
  | { id?: string; action: 'viewerAnswer'; peerId: string; sdp: string }
  | { id?: string; action: 'viewerCandidate'; peerId: string; candidate: string; mid: string }
  | { id?: string; action: 'removeViewer'; peerId: string };
...
export function startSignalingServer(sfu: NativeSfuBridge, port = 8000): void {
  ...
  signalWss.on('connection', socket => {
    ...
    socket.on('message', raw => {
      try {
        const request = JSON.parse(raw.toString()) as SignalRequest;
