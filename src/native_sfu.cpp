...
#include <iostream>
#include <stdexcept>
#include <atomic> // 멀티스레드 안전한 카운터를 위해 추가
#include <string>
...
// cname 생성을 위한 전역 카운터 (스레드 안전)
std::atomic<int> g_cname_counter{0};
...
rtc::Description::Video buildSendOnlyH264Description(std::uint32_t ssrc) {
  ...
  // cname 만들 때마다 숫자 +1 증가
  std::string dynamic_cname = "webcodecs-libdatachannel-toy-" + std::to_string(++g_cname_counter);
  video.addSSRC(ssrc, dynamic_cname, "stream0", "video0");
  return video;
}
...
void NativeSfu::configureViewerTrack(
  ...
  // 1. 해당 트랙 설정 시점의 고유한 고유 cname 생성
  std::string dynamic_cname = "webcodecs-libdatachannel-toy-" + std::to_string(++g_cname_counter);
  std::cout << "[viewer:" << peer_id << "] Generated CNAME: " << dynamic_cname << "\n";

  auto desc = track->description();
  desc.clearSSRCs();
  // 2. SDP 설정에 동적 cname 적용
  desc.addSSRC(viewer->ssrc, dynamic_cname, "stream0", "video0");
  track->setDescription(desc);
  ...
  // 3. RTP 패킷타이저 설정에도 동일한 동적 cname 적용
  auto rtp_config = std::make_shared<rtc::RtpPacketizationConfig>(
      viewer->ssrc,
      dynamic_cname,
      viewer->payload_type,
